...
async def print_invoice_gst(
    ...
    # Generate html template based on the batches of 17 items per batch
    batches = [items[i:i + 17] for i in range(0, no_of_items, 17)]
    template_vars["batches"] = batches
    template_vars["no_of_batches"] = len(batches)
    
    
    async with aiofiles.open("app/utils/templates/gst_invoice_template2.html", "r") as f:
        template_str = await f.read()
