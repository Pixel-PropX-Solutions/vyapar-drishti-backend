...
class CompanyRepo(BaseMongoDbCrud[CompanyDB]):
    def __init__(self):
        super().__init__(
            ENV_PROJECT.MONGO_DATABASE, "Company", unique_attributes=["company_name", "user_id"]
        )
...
class BillingRepo(BaseMongoDbCrud[BillingDB]):
    def __init__(self):
        super().__init__(
            ENV_PROJECT.MONGO_DATABASE, "Billing", unique_attributes=["address_1", '_id']
        )

    async def new(self, sub: Billing):
        return await self.save(BillingDB(**sub.model_dump()))

billing_repo = BillingRepo()


class ShippingRepo(BaseMongoDbCrud[ShippingDB]):
    def __init__(self):
        super().__init__(
            ENV_PROJECT.MONGO_DATABASE, "Shipping", unique_attributes=["address_1", '_id']
        )

    async def new(self, sub: Shipping):
        return await self.save(ShippingDB(**sub.model_dump()))

shipping_repo = ShippingRepo()